...
try:
    ...
    print("ERROR: lxml not installed. Run: python -m pip install -r .claude/skills/hwpxskill/scripts/requirements.txt", file=sys.stderr)
    sys.exit(1)
...
def get_text(el):
    # itertext()로 읽는다 — `.text`만 보면 탭 뒤에 오는 글자를 통째로 놓친다.
    # 서식의 `<hp:t><hp:tab/>    멘토명 : ...</hp:t>` 같은 줄이 그 자리이고,
    # 조용히 사라지므로 "그 말이 문서에 없다"는 오판으로 이어진다.
    # (실측 2026-09-01, 멘토일지 1건: `.text`만 834자·'멘토명' 미검출 / itertext 985자·검출)
    # 같은 처리를 page_guard.py의 _text_of_t_node()가 이미 하고 있다.
    texts = []
