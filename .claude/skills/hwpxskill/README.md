...
> 2026-09-01 덧: 이 판단은 **문서를 쓸 때**의 이야기입니다. **읽기**는 이미 `python-hwpx`를 씁니다(`text_extract.py`가 그 라이브러리의 `TextExtractor`를 감싼 것). 같은 날 실측에서 그 라이브러리는 무편집 왕복이 바이트 단위로 같았고, 읽기 함수는 어느 것을 부르냐에 따라 결과가 갈렸습니다 — 자세한 건 SKILL.md 「항상 지키는 것」. **쓰기 쪽을 갈아탈지는 아직 안 재봤습니다**(한글로 열어 비교할 윈도우 기계가 필요).
...
```bash
python scripts/build_hwpx.py --template gonmun --output result.hwpx
```
...
```bash
python scripts/office/unpack.py document.hwpx ./unpacked/
# XML 수정
python scripts/office/pack.py ./unpacked/ edited.hwpx
```
...
```bash
python scripts/text_extract.py document.hwpx --format markdown
```
...
- Python 3.6 이상
- 가상환경 (워크스페이스 `.venv`)
- 패키지: `scripts/requirements.txt` — 설치는 SKILL.md 「환경 › 설치」
