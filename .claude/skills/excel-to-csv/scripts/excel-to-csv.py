...
def convert_sheet_to_csv(ws, output_path: str, sheet_name: str,
    ...
    if data_rows <= 0:
        # 표가 아닌 시트(메모·표지처럼 한두 줄짜리)가 여기서 조용히 사라졌다.
        # 첫 비어있지 않은 줄을 헤더로 잡고 나면 그 아래 데이터가 없어 "empty"가 되는데,
        # 종료코드는 0이고 "skipped (empty)"만 찍혀서 시트 수를 세지 않으면 유실을 모른다.
        # 내용이 있으면 표가 아니더라도 그 줄들을 그대로 내보낸다.
        head_ws_row = effective_start + 1
        rows_out = []
        for row in ws.iter_rows(min_row=head_ws_row,
                                max_row=max(max_row + 1, head_ws_row),
                                min_col=1, max_col=max_col, values_only=True):
            vals = [format_cell(c) for c in row]
        ...
        return {"sheet": sheet_name, "rows": len(rows_out), "path": output_path,
                "skipped": False, "non_tabular": True}
...
def convert_excel(file_path: str, output_dir: str, sheet_name: str = None,
    ...
    for r in results:
        ...
        elif r.get("non_tabular"):
            print(f"  [{r['sheet']}] -> {Path(r['path']).name} "
                  f"({r['rows']} rows, 표 형태가 아니라 내용 그대로 옮김)")
            converted_count += 1
            total_rows += r["rows"]
        else:
