...
### Phase C — 높이 검증은 이 표가 아니라 실측이다

이 문서의 mm 값은 Phase B 초안 배치용 추정이다. 검증은 HTML을 만든 뒤 `python .claude/skills/md-to-pdf/scripts/measure_pages.py <handout.html>`로 실측한다 (SKILL.md Step 4 Phase C가 원본). 아래 구간은 실측값을 받은 뒤의 조치 기준이다.
...
- [ ] `measure_pages.py` 실측이 넘침 0건인가 (예상 높이 합산으로 대신하지 않는다)
