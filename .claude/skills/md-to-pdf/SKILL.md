...
구조 우선, 높이 검증. 높이 **추정**(초안 배치용)은 [references/content-budget.md](references/content-budget.md) 참조 — **검증은 추정이 아니라 실측**(아래 Phase C).
...
**Phase C — 높이 검증 (실측 게이트 — 눈대중 금지)**

content-budget 표는 Phase B의 초안 배치용이다. 검증은 HTML을 만든 뒤 실측한다:

```bash
python .claude/skills/md-to-pdf/scripts/measure_pages.py <handout.html>
```

- 페이지마다 콘텐츠/가용 높이(mm)·채움률·넘침을 렌더해 잰다. `.page`가 `overflow:hidden`이라 넘침은 화면에선 잘림으로만 보여 눈으로 못 잡는다
- **넘침(rc=1) 0건까지 재배치** — 위반 페이지는 블록별 실측 mm가 같이 나오므로 어느 그룹을 옮길지 계산으로 정한다
- 실측 조치 기준: **≤ 233mm** OK / **233-240mm** divider margin 축소(10→5mm) 등 미세 조정 / **> 240mm** 마지막 그룹을 다음 페이지로
...
- [ ] 페이지 번호 순차, 총합 정확
- [ ] `measure_pages.py` 실측이 넘침 0건 (추정 합산으로 대신하지 않는다)
