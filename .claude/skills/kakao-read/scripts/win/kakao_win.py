...
def to_local(winpath):
    ...
    return winpath
# 매핑 원본은 ../kakao_read.py의 TYPE_MASK/TYPE_LABEL — 값을 늘리려면 그쪽을 고친다.
# type 상위 비트는 플래그라 완전일치로 비교하면 플래그 붙은 메시지가 통째로 빠진다
# (실측 2026-08-26: 16385 = 1 일반 텍스트 653건). 그래서 볼 때 하위 14비트로 마스킹한다.
TYPE_MASK = 16383
...
def _rval(buf, o, t):
    n = _slen(t)
    # 경계 초과면 값을 만들지 않는다. 파이썬 슬라이스는 범위를 넘어도 에러 없이
    # 짧게 자르는데, 그러면 정수는 틀린 값·문자열은 잘린 값이 조용히 나온다
    # (실수 타입만 struct가 에러를 낸다). 쓰레기 메모리를 훑는 파서라 경계 초과는
    # 예외가 아니라 상시 입력이므로 여기서 거른다.
    if o + n > len(buf): return None, o + n
    raw = buf[o:o+n]
...
def parse_leaf(page):
    ...
    for c in range(ncell):
        ...
        for t in types:
            if vo + _slen(t) > len(page): ok = False; break
...
def fmt_msg(sendAt, authorId, typ, message, users, own_id=None):
    ...
    label = TYPE_LABEL.get(typ & TYPE_MASK, None)
    if label is not None:
        body = label + ((" " + message) if ((typ & TYPE_MASK) == 18 and message) else "")
    elif message:
...
def ensure_procdump():
    ...
    if target.exists():
        os.chmod(target, 0o755)   # 옛 버전이 권한 없이 받아둔 파일도 여기서 살린다
        return str(target)
    ...
    try:
        ...
        with z.open(pick) as src, open(target, "wb") as dst:
            dst.write(src.read())
        # zip에서 푼 파일은 권한 644라 WSL에서 실행이 안 된다(리눅스 FS의 .exe는
        # 실행 비트가 필요). /mnt/c 아래 procdump를 쓰던 기계에선 드러나지 않는 자리.
