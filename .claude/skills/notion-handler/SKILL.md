...
**최초 1회 — 파이썬 환경**: 이 스킬은 `requests` 패키지를 쓴다. 워크스페이스 가상환경에 설치한다.

```bash
source .claude/venv.sh        # 가상환경 활성화 (Mac·윈도우 Git Bash 공통)
python -m pip install requests
```

아래 명령은 모두 이 활성화가 된 상태를 전제로 한다. 활성화 없이 부르면 `ERROR: requests not installed` 로 멈춘다.
...
#### DB 생성
```bash
python .claude/skills/notion-handler/scripts/notion_api.py create-db \
  --parent "페이지ID" \
  --title "DB 제목" \
...
#### DB 조회
```bash
python .claude/skills/notion-handler/scripts/notion_api.py query-db \
  --id "DB_ID" \
...
#### DB 정보 확인
```bash
python .claude/skills/notion-handler/scripts/notion_api.py get-db --id "DB_ID"
```
...
#### 페이지 생성 (DB 항목)
```bash
python .claude/skills/notion-handler/scripts/notion_api.py create-page \
  --parent "DB_ID" \
...
#### 페이지 조회
```bash
python .claude/skills/notion-handler/scripts/notion_api.py get-page --id "PAGE_ID"
```

#### 페이지 수정
```bash
python .claude/skills/notion-handler/scripts/notion_api.py update-page \
  --id "PAGE_ID" \
  --properties '{"상태": "완료"}'
```
...
#### 블록 추가
```bash
python .claude/skills/notion-handler/scripts/notion_api.py append-blocks \
  --id "PAGE_ID" \
...
#### 블록 조회 (기본)
```bash
python .claude/skills/notion-handler/scripts/notion_api.py get-blocks --id "PAGE_ID"
```

#### 블록 조회 (페이지네이션 포함)
```bash
# 모든 블록 가져오기
python .claude/skills/notion-handler/scripts/notion_api.py get-all-blocks --id "PAGE_ID"

# 중첩 블록까지 재귀적으로 가져오기
python .claude/skills/notion-handler/scripts/notion_api.py get-all-blocks --id "PAGE_ID" --recursive
```

#### 블록 수정
```bash
python .claude/skills/notion-handler/scripts/notion_api.py update-block \
  --id "BLOCK_ID" \
...
```bash
# 단일 블록 삭제
python .claude/skills/notion-handler/scripts/notion_api.py delete-block --id "BLOCK_ID"

# 여러 블록 삭제
python .claude/skills/notion-handler/scripts/notion_api.py delete-blocks \
  --ids '["BLOCK_ID_1", "BLOCK_ID_2", "BLOCK_ID_3"]'

# 페이지 전체 블록 삭제 (페이지 초기화)
python .claude/skills/notion-handler/scripts/notion_api.py clear-page --id "PAGE_ID"
```
...
```bash
python .claude/skills/notion-handler/scripts/notion_api.py search --query "검색어"

# 페이지만 검색
python .claude/skills/notion-handler/scripts/notion_api.py search --query "검색어" --type page

# 데이터베이스만 검색
python .claude/skills/notion-handler/scripts/notion_api.py search --query "검색어" --type database
```
