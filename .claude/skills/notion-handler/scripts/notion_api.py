...
def _from_schema(ptype, value):
    """스키마 타입 → Notion 속성 객체. 모르는 타입(relation·formula 등)은 None → 휴리스틱으로."""
    if ptype == "title":
...
def parse_page_properties(properties: Dict[str, Any], schema: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    for name, value in properties.items():
        if isinstance(value, dict):
            # Notion 원본 속성 객체는 변환 없이 통과
            result[name] = value
        elif schema and _from_schema(schema.get(name, {}).get("type"), value) is not None:
            # 스키마가 타입을 아는 속성 — 추측하지 않는다 (2026-08-26, select 오변환 사고 처방)
            result[name] = _from_schema(schema[name]["type"], value)
        elif name == "이름" or (schema and schema.get(name, {}).get("type") == "title"):
            # Title property
            result[name] = {
...
def create_page(parent_id: str, properties: Dict[str, Any],
    ...
    if is_database:
        ...
        try:
            # 값 모양 추측 대신 DB 스키마로 변환한다 (2026-08-26 select 오변환 사고 처방)
            schema = get_database(parent_id).get("properties") or None
        except Exception:
            schema = None  # 조회 실패 시 휴리스틱으로 — 생성 자체를 막지 않는다
    else:
        parent = {"type": "page_id", "page_id": format_id(parent_id)}
