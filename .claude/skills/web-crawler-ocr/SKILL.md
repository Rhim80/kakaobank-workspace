...
This skill activates only when the user explicitly indicates **save/archive/OCR intent**:
- "이 URL 워크스페이스에 저장해줘", "아카이빙해줘", "파일로 뽑아줘"
- "이미지 많은 상세페이지라 OCR 필요해"
- 여러 URL 동시 제공 + 배치 저장 의도
- "large images on this page, bypass 5MB limit"
...
# 워크스페이스 루트에서 실행 (cd 하지 않는다 — 출력 경로가 어긋난다)
python3 -m venv .claude/skills/web-crawler-ocr/scripts/venv        # 윈도우 Git Bash엔 python3 가 없을 수 있다 → python 또는 py
source .claude/venv.sh .claude/skills/web-crawler-ocr/scripts/venv
python -m pip install -r .claude/skills/web-crawler-ocr/scripts/requirements.txt && \
python -m pip install playwright && playwright install chromium

# 2. Run crawler
source .claude/venv.sh .claude/skills/web-crawler-ocr/scripts/venv && \
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py "<URL>" "<output-path>"

# 3. Dynamic/SPA sites (Olive Young, Coupang, Musinsa etc.)
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py "<URL>" "<output-path>" --wait 5000 --scroll --max-images 10
```
...
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> [output.md]

# Dynamic/SPA sites (recommended for Korean e-commerce)
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> --wait 5000 --scroll --max-images 10

# JS rendering wait only (no scroll)
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> --wait 3000

# Force stealth mode (Cloudflare bypass)
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> --stealth

# Text only, no OCR
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> --no-ocr

# Limit OCR images
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py <URL> --max-images 5
```
...
source .claude/venv.sh .claude/skills/web-crawler-ocr/scripts/venv && \
python .claude/skills/web-crawler-ocr/scripts/web-crawler.py \
...
```bash
source .claude/venv.sh .claude/skills/web-crawler-ocr/scripts/venv && \
python -m pip install -r requirements-stealth.txt && \
scrapling install
```
...
```bash
python -m pip install -r requirements-stealth.txt && scrapling install
```
...
```bash
# 워크스페이스 루트에서
rm -rf .claude/skills/web-crawler-ocr/scripts/venv
python3 -m venv .claude/skills/web-crawler-ocr/scripts/venv        # 윈도우는 python 또는 py
source .claude/venv.sh .claude/skills/web-crawler-ocr/scripts/venv
python -m pip install -r .claude/skills/web-crawler-ocr/scripts/requirements.txt
```
