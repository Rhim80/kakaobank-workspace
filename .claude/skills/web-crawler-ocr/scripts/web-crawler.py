...
try:
    ...
    S = ".claude/skills/web-crawler-ocr/scripts"
    print(f"ERROR: 의존성 미설치 ({e.name}). 워크스페이스 루트에서 아래를 실행하세요:", file=sys.stderr)
    print(f"  python3 -m venv {S}/venv          # 윈도우는 python 또는 py", file=sys.stderr)
    print(f"  source .claude/venv.sh {S}/venv", file=sys.stderr)
    print(f"  pip install -r {S}/requirements.txt", file=sys.stderr)
