...
> Custom slash commands는 Skills로 통합되어 `skills-guide.md`가 함께 다룬다 (별도 문서 없음).
...
3. **동기화 상태 파생 (json을 믿지 않는다)**:
   ```bash
   python .claude/skills/doc-updater/scripts/sync_doc_versions.py
   ```
   - 가이드 5종 헤더의 `**Updated**: … (vX.Y.Z)`에서 `docs_synced_version`을 **파생**하고 json을 재생성한다
   - json을 직접 읽고 시작하지 말 것 — 갱신이 누락된 json은 이미 반영된 구간을 재훑거나 문서를 되돌리게 만든다. 헤더가 원본, json은 캐시다
...
1. **버전 추적 파일 갱신 (파생 — 손으로 값을 적지 않는다)**:
   ```bash
   python .claude/skills/doc-updater/scripts/sync_doc_versions.py
   ```
   - Step 3에서 갱신한 문서 헤더가 곧 동기화 버전이다 — 스크립트가 헤더를 파싱해 json을 다시 쓴다
   - 이 단계를 잊어도 다음 실행의 Step 1이 같은 스크립트로 재파생하므로 값이 틀린 채 남지 않는다
