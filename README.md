...
2. **Python 확인** — 카톡 읽기(윈도우)·배선 검사에 쓰는 파이썬 3(3.9 이상)이 있는지만 확인 (설치할 패키지 없음)
...
“배선도 그려줘”라고 말하고 사전 업무파악 응답을 붙여 주세요. Claude Code가 줄기와 연결을 확인한 뒤 YAML 선언·수집 스킬·병목 스킬을 만듭니다. 설정 원본은 `00-system/wiring/선언.yaml`이며, “아침 시작하자”로 수집과 승인 대기 처리를 이어갑니다. YAML을 직접 작성할 필요는 없습니다.
...
할 일·하루 노트·진행·주간·위키 위치는 `00-system/선언-표면.yaml`에 선언합니다. 스킬별로 경로를 다시 적지 않고 같은 선언을 읽습니다.
