...
> **Updated**: 2026-08-21 (v2.1.238) - 내장 `claude-api` 스킬 컨텍스트 비용 ~25k로 축소(참조 문서 on-demand)·claude.ai 동기화 스킬 강화(로컬 커맨드 shadow 금지·`!`/`@` 미확장)·`claude plugin validate`의 bare `.claude/skills` 검사·번들 별칭 `-p` 모드 호출 수정·인자 치환 재확장 수정·포크 스킬 모델 제한 경고 반영
...
### `disableBundledSkills` 설정 (v2.1.169)

**(v2.1.169)** `settings.json`에 `disableBundledSkills` 설정 추가. Claude Code에 기본 내장된 번들 스킬을 모델에게서 숨긴다. 커스텀 스킬만 노출하고 싶을 때 사용.

### 스킬 연속 호출 (스택) (v2.1.199)

**(v2.1.199)** `/skill-a /skill-b do XYZ`처럼 슬래시 스킬을 여러 개 앞에 이어서 붙이면 **앞쪽 스킬들이 모두 로드**됨 (첫 번째 하나만이 아니라 최대 5개). 여러 스킬을 순차로 조합해 한 프롬프트에서 실행할 때 사용.

### `/dataviz` 내장 스킬 (v2.1.198)

**(v2.1.198)** 차트·대시보드 디자인용 `/dataviz` 스킬이 기본 내장으로 추가됨.

### 번들 스킬 자동 실행 정책·번들 스킬 갱신 (v2.1.215~v2.1.238)

- **(v2.1.215)** Claude가 `/verify`·`/code-review` 스킬을 스스로 실행하지 않음 — 원할 때 `/verify`·`/code-review`로 직접 호출
- **(v2.1.218)** `/deep-research`도 수동 호출 시에만 시작 (Claude가 자체 판단으로 실행하지 않음)
- **(v2.1.218)** `/code-review`는 백그라운드 서브에이전트로 실행됨 — 리뷰 작업이 대화 컨텍스트를 채우지 않고, 스택된 슬래시 커맨드를 리뷰 대상으로 유지
- **(v2.1.219)** 번들 `claude-api` 스킬이 Claude Opus 5를 기본으로 안내하도록 갱신 (Opus 4.8에서의 마이그레이션 경로 포함)
- **(v2.1.221)** `claude-api` 스킬에 `prompt-audit` 서브커맨드 추가 — 프롬프트와 도구 설명에서 구형 모델용으로 쓰인 패턴을 점검
- **(v2.1.216)** 번들 `dataviz` 스킬 갱신: 기본 차트 팔레트 순서 조정, 4계열 차트에 직접 라벨을 권하던 가이드 수정
- **(v2.1.234)** 내장 `claude-api` 스킬의 **로딩 컨텍스트 비용이 ~200k+ 토큰에서 ~25k로 감소** — 참조 문서를 필요할 때만 읽도록 변경
- **(v2.1.238)** 번들 `claude-api` 스킬이 Managed Agents 8/19 릴리스에 맞춰 갱신 (web search·fetch 도메인 설정, self-hosted 샌드박스의 메모리 스토어)
- **(v2.1.232)** `/code-review`가 high·xhigh·max 레벨에서도 다른 레벨과 동일하게 **백그라운드 에이전트로 실행**됨
- **(v2.1.233 수정)** 사용자·프로젝트 스킬이 번들 스킬을 가릴 때, 번들 별칭(`/checkup`·`/review`)이 `-p` 모드나 플러그인·MCP 로드 상태에서 "Unknown command"로 뜨던 버그 수정

### claude.ai 동기화 스킬 강화 (v2.1.228)

**(v2.1.228)** claude.ai에서 동기화된 스킬은 로컬 커맨드나 MCP 프롬프트를 **가릴 수 없고**, description이 sanitize되어 출처 라벨이 붙으며, 내 기계에서는 본문의 `!` 명령 실행과 `@` 파일 확장이 **동작하지 않는다**. 함께 스킬 호출 뒤 deferred-tools 리마인더가 모델에 두 번 전달되던 버그도 수정됨.

### 스킬 인자 치환·검증 (v2.1.233)

- **(v2.1.233)** 스킬·커맨드 인자 치환에서 **인자 값이 다시 템플릿 마커로 확장되지 않도록** 수정
- **(v2.1.233)** `claude plugin validate`가 `.claude/skills` 디렉토리만 있는 경우도 검사해, frontmatter 파싱에 실패하는 `SKILL.md`를 보고

### 포크 스킬의 서브에이전트 모델 제한 경고 (v2.1.223)
...
- 라이브 변경 감지: 세션 재시작 없이 SKILL.md 편집 즉시 반영. **(v2.1.216 수정)** 세션 중 변경된 스킬·커맨드가 재시작 없이 슬래시 메뉴에도 나타나도록 수정
...
- **(v2.1.178)** 중첩 `.claude/skills` 디렉토리의 스킬은 해당 위치 파일을 작업할 때 로드됨. 중첩 `.claude/` 디렉토리에서 이름이 충돌하면 가장 가까운(closest) agent/workflow/output-style이 승리.

### Frontmatter 키 표기 관대화 (v2.1.186)

**(v2.1.186)** `display-name`, `default-enabled`, `fallback`, `metadata.*` frontmatter 필드가 kebab/snake/camelCase 표기를 모두 허용. 또한 `SKILL.md` YAML frontmatter가 잘못 작성된 경우, 메타데이터를 비운 채로 스킬 본문을 로드 (이전에는 스킬 전체 로드 실패).
...
| `background` | - | `true` (fork 시) | **(v2.1.218)** `context: fork` 스킬은 기본 백그라운드 실행. `background: false`로 동기 실행 옵트아웃. |
...
**(v2.1.222)** `disable-model-invocation` 스킬을 Claude가 호출하려 하면, 거부 메시지가 **"사용자에게 그 스킬을 직접 실행해 달라고 요청하라"**고 지시함. 이전처럼 Claude가 스킬 워크플로우를 자기 나름대로 재현(replicate)하지 않도록 하는 변경.
...
| `\$` | **(v2.1.163)** 커맨드 본문에서 숫자 앞에 리터럴 `$`를 넣을 때 사용하는 escape 문법. 예: `\$1`은 변수 치환 없이 `$1` 문자 그대로 출력. **(v2.1.210 수정)** 매칭되지 않은 위치 인자 플레이스홀더(`$1` 등에 해당 인자 없음)가 조용히 제거되던 버그 수정 — 이제 보존됨. |
...
- **(v2.1.218)** `context: fork` 스킬은 **기본으로 백그라운드에서 실행**됨. 동기(foreground) 실행이 필요하면 해당 스킬 frontmatter에 `background: false` 지정.
...
| SessionStart | **(v2.1.152)** 세션 시작/재개 시. `reloadSkills: true`로 스킬 재스캔, `hookSpecificOutput.sessionTitle`로 세션 제목 설정 가능. **(v2.1.214)** fork로 시작된 세션에서는 source를 `"fork"`로 보고 |
...
12. **(v2.1.202)** 스킬을 다시 호출할 때 인스트럭션이 중복으로 주입되던 버그 수정
13. **(v2.1.211)** 동기화된 스킬 디렉토리의 이름 처리가 Windows에서 강화됨
14. **(v2.1.217)** `SKILL.md`(또는 `CLAUDE.md`)의 `paths` frontmatter에 brace 그룹(`{a,b}`)이 많으면 CLI가 시작 시 OOM/멈춤 발생하던 버그 수정 — brace 확장이 예산 제한(budget-bounded)됨
15. **(v2.1.216)** 세션 도중 추가·수정한 스킬과 커맨드가 재시작 전까지 슬래시 메뉴에 안 나오던 버그 수정. 함께 `name` frontmatter가 있는 플러그인 스킬이 슬래시 자동완성에서 플러그인 접두사를 잃던 버그 수정
16. **(v2.1.221)** 플러그인·조직 배포 스킬이 터미널 전용 빌트인(`/help`·`/feedback` 등)과 같은 이름이면 **비대화형 세션에서 호출 불가**하던 버그 수정
17. **(v2.1.221)** 플러그인이 `skills` 경로로 `"."`(플러그인 루트)을 받도록 변경 — 루트에 `SKILL.md`를 둔 경우의 검증 에러가 플러그인 루트 사용을 안내함
